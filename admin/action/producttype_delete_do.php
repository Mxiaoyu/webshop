 ...
 /* 商品类别删除处理页 */  
  ...
  if( $_GET["PT_ID"]!= "")
  {
    ...
    else 
    {
      ...
      if($delete)                 //判断执行结果
      {
    	echo "<script>alert('商品类别删除成功！');window.location.href='../producttype_manager.php'</script>";
      }
      else
      {
        echo "<script>alert('商品类别删除失败！');window.location.href='../producttype_manager.php'</script>";
      }
    } 
  }
  else 
  {
   echo "<script>alert('请选择要删除的商品类别！');window.location.href='../producttype_manager.php'</script>";     
  }
 ?>